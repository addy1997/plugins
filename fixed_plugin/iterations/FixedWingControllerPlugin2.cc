...
{ 
   ...
   // Create joint velocity component only if one doesn't exist
   auto jointVelComp = 
       _ecm.Component<components::JointVelocity>(this->joints[this->kPropeller]);
   ...
   double targetVel;
   {
      ...
      targetVel = this->jointVelCmd;
   }
   
   // Force mode.
   if (this->cmds[this->kPropeller])
   {
      double maxVelocity = this->propellerMaxRpm*2.0*IGN_PI/60.0;

      double error = (jointVelComp->Data().at(0) + maxVelocity * this->cmds[kPropeller])
                              - targetVel;
      double force = this->propellerPID.Update(error, _info.dt);
   
      auto jointForceComp =
        _ecm.Component<components::JointForceCmd>(this->cmds[this->kPropeller]);

      if (jointForceComp==nullptr)
        return;
   }
   ...
   {
      // Create joint position component only if one doesn't exist
      auto jointPosComp = 
         _ecm.Component<components::JointPosition>(this->joints[i]);
      ...
      {
         std::lock_guard<std::mutex> lock(this->mutex);
         error = jointPosComp->Data().at(i) - this->cmds[i];
      }

      double force = this->controlSurfacesPID[i].Update(error, _info.dt);
      auto jointForceComp = 
            _ecm.Component<components::JointForceCmd>(this->joints[i]);
      if (jointForceComp == nullptr)
         return;
     }
}
